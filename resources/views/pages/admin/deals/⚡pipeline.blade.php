...
new class extends Component
{
    ...
    protected function searchQuery()
    {
        ...
        // Sales Executive owns ASSIGNED opportunities only — see deals/all.blade.php.
        $user = auth()->user();
    ...
    {
        // 'active' = still open / in the pipeline, as opposed to won, lost,
        // on_hold, or cancelled (see the deal_status validation rules on
        // the Add/Edit pages for the full enum).
        return $this->searchQuery()->where('deal_status', 'active');
    }
    ...
    {
        return [
            'total' => (clone $this->searchQuery())->count(),
            'active' => (clone $this->searchQuery())->where('deal_status', 'active')->count(),
            'won' => (clone $this->searchQuery())->where('deal_status', 'won')->count(),
            'lost' => (clone $this->searchQuery())->where('deal_status', 'lost')->count(),
        ];
    }
