...
new class extends Component
{
    ...
    {
        abort_unless(auth()->user()->role === 'admin' || auth()->user()->hasPermission('Staff', 'Edit'), 403);
        $this->member = staff::findOrFail($id);
        $this->staffId = $this->member->id;
        $this->name = $this->member->name;
        $this->email = $this->member->email;
        $this->whatsapp = $this->member->whatsapp;
        $this->aadhar = $this->member->aadhar;
        $this->pan = $this->member->pan;
        $this->designation = $this->member->designation;
        $this->employment_type = $this->member->employment_type ?: 'full_time';
        $this->shift_start = $this->member->shift_start ?: '09:00';
        $this->daily_hours = $this->member->daily_hours ?: 8;
        $this->joining_date = optional($this->member->joining_date)->format('Y-m-d');
        $this->tenure_start = optional($this->member->tenure_start)->format('Y-m-d');
        $this->tenure_end = optional($this->member->tenure_end)->format('Y-m-d');
        $this->salary = $this->member->salary;
        $this->status = $this->member->status ?? 'active';
    ...
    public function update()
    {
        ...
        if ($this->photo) {
            ...
            $this->member->image = $this->photo->store('staffs', 'public');
        }

        $this->member->name = $this->name;
        $this->member->email = $this->email;
        $this->member->whatsapp = $this->whatsapp;
        $this->member->aadhar = $this->aadhar ?: null;
        $this->member->pan = $this->pan ?: null;
        $this->member->designation = $this->designation;
        $this->member->employment_type = $this->employment_type ?: 'full_time';
        $this->member->shift_start = $this->shift_start ?: '09:00';
        $this->member->daily_hours = (int) ($this->daily_hours ?: 8);
        $this->member->joining_date = $this->joining_date;
        $this->member->tenure_start = $this->employment_type === 'intern' ? ($this->tenure_start ?: null) : null;
        $this->member->tenure_end = $this->employment_type === 'intern' ? ($this->tenure_end ?: null) : null;
        $this->member->salary = $this->salary;
        $this->member->status = $this->status ?: 'active';
        $this->member->save();

        session()->flash('success', 'Staff member updated successfully!');

        return redirect()->route('staff.show', $this->staffId);
    }
    ...
    <div class="dashboard">
        ...
        <div class="page-header">
            ...
            <div class="header-actions">
                <a href="{{ route('staff.show', $staffId) }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i> Back
                </a>
                <button type="button" wire:click="update" class="btn btn-primary">
                    <i class="fas fa-save" wire:loading.remove></i> <i class="fas fa-spinner fa-spin" wire:loading></i> Update Staff
                </button>
            </div>
        </div>
                    ...
                    <div class="row">
                                ...
                                <div class="row g-3">
                                    ...
                                    @if ($employment_type === 'intern')
                                        <div class="col-md-6">
                                            ...
                                            <input type="date" class="form-control" wire:model="tenure_start">
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label fw-medium"><i class="fas fa-hourglass-end me-1 text-muted"></i> Internship End</label>
                                            <input type="date" class="form-control" wire:model="tenure_end">
                                        </div>
                                    @endif
                                    <div class="col-md-6">
                                        <label class="form-label fw-medium">
                        ...
                        <div class="col-lg-4">
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    @if ($member->image && !$photo)
                                        <img src="{{ asset('storage/'.$member->image) }}" class="img-fluid rounded mb-3" alt="Current photo">
                                    @endif
