...
new class extends Component
{
    ...
    {
        $this->plan = PricingPlan::findOrFail($id);
        $this->name = $this->plan->name;
        $this->service_category_id = $this->plan->service_category_id;
        $this->price = $this->plan->price;
        $this->billing_period = $this->plan->billing_period;
        $this->featuresText = implode("\n", $this->plan->features ?? []);
        $this->countriesText = implode("\n", $this->plan->countries ?? []);
    ...
    public function update()
    {
        ...
        $this->plan->service_category_id = $this->service_category_id ?: null;
        $this->plan->price = $this->price;
        $this->plan->billing_period = $this->billing_period ?: 'monthly';
        $this->plan->features = array_values(array_filter(array_map('trim', explode("\n", $this->featuresText))));
        $this->plan->countries = array_values(array_filter(array_map('trim', explode("\n", $this->countriesText))));
        $this->plan->status = $this->status ?: 'active';
        $this->plan->save();
    ...
    <div class="dashboard">
        ...
        <div class="page-header">
            <div>
                ...
                <p>Update pricing plan details.</p>
            </div>
            <div class="header-actions">
                <a href="{{ route('pricing.all') }}" class="btn btn-secondary">
                    <i class="fas fa-arrow-left"></i> Back to Pricing
                </a>
                <button type="button" form="pricingForm" wire:click="update" class="btn btn-primary">
                    <i class="fas fa-save" wire:loading.remove></i> <i class="fas fa-spinner fa-spin" wire:loading></i> Update Plan
                </button>
            </div>
        </div>
